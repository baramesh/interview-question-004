...
# API-PF-02 — GET /api/occupations

## สรุป

ส่งรายการข้อมูลหลักอาชีพที่ใช้งานให้หน้า Angular สำหรับสร้างตัวเลือก โดยเรียงตาม `display_order` แล้วตาม `name` หน้าเว็บแสดง `name` และเก็บ `code` เป็นค่าที่ส่งกลับใน `POST /api/profiles`

## Caller / Consumer

| caller kind      | caller refs | trigger              | use                                                  |
| ---------------- | ----------- | -------------------- | ---------------------------------------------------- |
| `ui-interaction` | `UIX-PF-01` | เปิดหน้าสร้างโปรไฟล์ | เติม combo box อาชีพโดยไม่เขียนรายการตายตัวใน Client |

## Security

Endpoint เป็น public เฉพาะสภาพแวดล้อมทดสอบในเครื่องตาม `SV-PF-01` และไม่รับ access token
...
ตัวอย่าง: `examples/api-pf-02-get-occupations/occupation-list-response.example.yaml`
...
| --------------- | ----------------------------------------------------------- |
| `UT-API-PF-009` | ส่งเฉพาะข้อมูลหลักที่ใช้งานและเรียงตาม `display_order`      |
| `TC-PF-E2E-008` | API ส่งรายการตามลำดับและหน้าเว็บแสดง `name` ตรงกับ response |
